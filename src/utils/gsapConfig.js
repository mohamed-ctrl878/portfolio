...
export class AnimationUtils {
    ...
    static staggerText(selector, delay = 0.1) {
    ...
    static revealOnScroll(selector, options = {}) {
        const defaults = {
            y: 100,
            opacity: 0,
            duration: 1,
    ...
    // Parallax effect
    static parallax(selector, speed = 0.5) {
        return ScrollTrigger.create({
            trigger: selector,
            start: "top bottom",
            end: "bottom top",
            scrub: true,
            animation: gsap.fromTo(selector, { y: -100 * speed }, { y: 100 * speed })
        });
    }
    // Magnetic hover effect
    static magnetic(selector, strength = 0.3) {
        const elements = gsap.utils.toArray(selector);
    ...
    static pageTransition(onComplete) {
